...
//	freopen("output.txt", "w", stdout);
	...
	while (t--) {
		...
		for (int i = 1, cd = 0; i <= n; ++i) { // count by increasing the depth by one 
			if (!(i & (i - 1))) ++cd;
			ld += cd - 1;
		}

		if (!(ld <= d && d <= rd)) { // the sum will always be min for an almost complete binary tree and highest for a skewtree
			cout << "NO" << endl;
			continue;
		}
